...
For $\alpha<1/e$, $f(\alpha)=-\alpha\log\alpha$ is an increasing function.
...
$f(\alpha)<1/e$. 
    \begin{align*}
    H(Y_i) &\leq \frac{3}{e}+\sum_{x=3}^n -\frac{1}{x!} \cdot\log_2(\frac{1}{x!})\\
    &= \frac{3}{e}+\sum_{x=3}^n \frac{\log_2(x!)}{x!}\\
    &\leq \frac{3}{e}+\sum_{x=3}^n \frac{x \log_2 x}{x!}\\
    &\leq \frac{3}{e}+\sum_{x=3}^n \frac{x (x-1)}{x!}\\
    &= \frac{3}{e}+\sum_{x=3}^n \frac{1}{(x-2)!}\\
    &\leq \frac{3}{e}+e-1\\
    & < 3
    \end{align*}
Combining this with the summation above gives $H(Y) \leq 3s$, so we get
	\begin{align*}
	H(X) &\leq H(Y)\\
	\log_2\left(\frac{k!}{(k-n)!}\right) &\leq 3s\\
...
For a game of mastermind on $n$ pegs and $k$ colors (with or without repeats), every
...
$Q_1, \ldots, Q_k$ which span $S$, any matrix $X\in S$,
including all hidden permutations, can be uniquely determined by the numbers
$X\cdot Q_i$ for $i\in [1,k]$.
