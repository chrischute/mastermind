...
\section*{Lower Bound on Guessing Effectiveness for Permutation Game}
	...
	\item
		...
		We give a proof by induction on $n$.
			...
			\end{enumerate}
		\end{proof}
	\item The minimax algorithm is a guessing strategy described by
	Knuth [DK76].
	At each turn, the algorithm ranks the possible guesses based on the fewest
	number of solutions that they would eliminate over all possible responses. It
	then picks the guess with the highest rank.\\\\
...
\section*{Extension of Previous Section to Repeated Colors}
	An analogous result for the above section can be easily shown with guesses
	eliminating a factor of $n^2k$ possible solutions at each step. However, in order to
	replicate the bound of $nk$ with repeated colors, we need to make the following assumption:
	...
	only the parts that differ from the previous proof.
		...
		When $k\ge n$, we have
			...
			& \le \frac{k-1}{k}
			\end{align*}
		Therefore, we always eliminate a fraction at least $1/k\ge 1/nk$
		of the solutions.
		...
		though, as by the induction hypothesis we can therefore eliminate
			\begin{align*}
			\frac{1}{(n-1)k}|D_{\sigma_1}|
			& \ge \frac{1}{(n-1)k}\frac{n-1}{n}|B_j|\\
			& = \frac{1}{nk}|S|
			\end{align*}
		...
		$(n-1)$-vectors will have
		exactly 1 fewer hit than they did as $n$-vectors, (as all of them had a hit
		in the first spot), so all of them fall into the same bucket (bucket $j-1$)
		for the guess $\{\sigma_2, \sigma_3, \ldots, \sigma_n\}$,
