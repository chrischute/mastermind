...
% TODO: Possibly add some history of Mastermind.
...
\begin{enumerate}[label=]
	\item\textbf{Theorem 1.} Knuth's minimax algorithm takes at most $nk+1$ turns to win Mastermind.
\end{enumerate}
Knuth's minimax algorithm is notable because it is, experimentally, the best method we have to solve small games of Mastermind in as few guesses as possible. However, by its nature, it is hard to analyze its asymptotic performance. Its behavior is determined by the distribution of solutions throughout the game, which proves very difficult to analyze \cite{KT86} \cite{OS13}. This bound is, to our knowledge, the first upper bound on the worst-case performance of the minimax algorithm, but if it performs near-optimally for large $n$ and $k$, we would expect this bound to be much smaller. We know, for example, that $f(n,k) = O(n \log k)$ for $k$ not too large. \cite{DK76} \cite{KT86}
% TODO: Check the above citations to see if correct.
...
\begin{enumerate}[label=]
	...
	\end{equation*}
\end{enumerate}
This theorem was proved by Doerr et al for the case $R = T$ , $k \ge n$ \cite{DS13}. They use an information-theoretic argument based on Shannon entropy by letting the hidden sequence be randomly chosen. 
...
These lower bounds together cover the case $n \geq k$. For neither choice of $R$ do we have tight upper bounds; when $R = T$ a corollary of a result by Doerr et al. \cite{DS13} gives an upper bound of $O(k \log k)$ guesses, and for $R = F$ no improvement over the $nk$ bound is known.
...
\begin{proof}
	...
	Let the hidden vector $Z$ be sampled uniformly at random from the set of $k!/(k-n)!$ possibilities. Then consider the random variables $Y_i = b(Z, q_i)$ be the responses for each guess. Then the vector $Y = (Y_1,Y_2,\ldots,Y_s)$ is a random variable and always uniquely determines, and is uniquely determined by, $Z$. So by Property 1, $N(Z) = N(Y)$. Since $Z$ is a random variable with $k!/(k-n)!$ outcomes of equal probability, we have
	\begin{equation}\label{equalEntropy}
		N(Z) = \log_2\left(\frac{k!}{(k-n)!}\right).
	\end{equation}
	
	By Property 2 of Shannon entropy, we know
	...
	\begin{equation}\label{entropyDef}
		N(Y_i)=-\sum_{x=0}^n \textnormal{Pr}[Y_i=x]\cdot\log_2(\textnormal{Pr}[Y_{i}=x]).
	\end{equation}
...
First, we note that \cite{DS13} showed the existence of an $O(n \log n)$ strategy for $k = n$. When $k>n$, we add ``imaginary spots'' until $n = k$, and fill them in with any arbitrary colors. Then, when we make our guesses, we adjust the responses accordingly to account for the imaginary spots. By \cite{DS13}'s result, we have a strategy that takes $O(k \log k)$ guesses.
% TODO: We should probably formalize this argument.
