...
This proof was inspired by [GK00].
	...
	\begin{proof}
	As in [GK00], we use the term ``critical pair'' to refer to a comparison between
    ...
    Two vectors are distinguised from one another if we've made at least one query
    for which the vectors received different responses. Thus the probability that
    there exists a pair of vectors that have not been distinguished from one another
    is given by the sum over all critical pairs of the probability that two vectors
    received the same response to all submitted queries. Let $s=4k\log k$ be
    the number of queries submitted. Then we have
    	\begin{align*}
		\text{Pr}[\exists\text{ pair } v_1, v_2 & \text{ not distinguished}]\\
    	& = \sum_{x=1}^n(\text{\# of critical pairs of size }x)
		\cdot\text{Pr}[\text{single pair not distinguished}]\\
        & = \sum_{x=1}^n \left[\binom{n}{x}k^x(k-1)^x
        \cdot\left(\sum_{i=0}^{\left\lfloor x/2 \right\rfloor}
		\textnormal{Pr}[\textnormal{Eq}(q,v_1) = i
    	\cap\textnormal{Eq}(q,v_2) = i]\right)^s ~\right]\\
        &\le\sum_{x=1}^n \binom{n}{x}k^x(k-1)^x
        \left(1-\frac{1}{k}\right)^{s\cdot x} \\
